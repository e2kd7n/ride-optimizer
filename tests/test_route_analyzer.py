...
# ---------------------------------------------------------------------------
# _dedupe_route_names (issue #445): disambiguation must be exhaustive, not
# "stop after the first pass that changes anything"
# ---------------------------------------------------------------------------

def _make_group_for_dedupe(group_id, distance, elevation_gain=85.0, name="Via Lakefront Trail — short"):
    """Build a RouteGroup whose only distinguishing feature is distance/elevation."""
    route = _make_route(int(group_id.split("_")[-1]) if group_id.split("_")[-1].isdigit() else 1,
                         distance=distance, elevation_gain=elevation_gain)
    return _make_group(group_id, routes=[route], name=name)


class TestDedupeRouteNames:
    def test_no_clash_left_untouched(self):
        """A single group with a unique name is never renamed."""
        groups = [_make_group_for_dedupe("g1", 14000.0, name="Home")]
        RouteAnalyzer._dedupe_route_names(groups)
        assert groups[0].name == "Home"

    def test_median_split_still_colliding_bucket_falls_through_to_ordinal(self):
        """
        Reproduces #445: 5 routes clustered within a fraction of a mile of each
        other. The short/long median split can only carve off one outlier, so
        4 of the 5 routes still land in the same 'long' bucket. Previously the
        code `continue`d past this and left all 4 sharing the identical name;
        now the ordinal pass must run *within* that bucket.
        """
        # meters, roughly matching the issue's 8.9 / 9.3 / 9.3 / 9.4 / 9.4 mi cluster
        distances = [14300.0, 14900.0, 14900.0, 15100.0, 15100.0]
        groups = [
            _make_group_for_dedupe(f"g{i}", d, name="Via Lakefront Trail → Home")
            for i, d in enumerate(distances, start=1)
        ]
        RouteAnalyzer._dedupe_route_names(groups)

        names = [g.name for g in groups]
        # No two groups may share a final name.
        assert len(names) == len(set(names))
        # The lone short-side outlier gets the 'short' bucket name.
        assert names[0] == "Via Lakefront Trail → Home — short"
        # The remaining 4 all fall in the 'long' bucket and must be
        # disambiguated further via ordinal suffixes.
        assert all("long" in n for n in names[1:])

    def test_terrain_split_still_colliding_bucket_falls_through_to_distance(self):
        """
        Same early-exit bug, but for Pass 1 (terrain): if a terrain bucket
        still has >1 member after the terrain split, distance-bracket (and if
        needed, ordinal) disambiguation must still run for that bucket.
        """
        groups = [
            # gain_per_km ~= 5 -> 'flat'
            _make_group_for_dedupe("g1", 12000.0, elevation_gain=60.0, name="Elm St"),
            # gain_per_km ~= 5 -> 'flat', but shorter distance than g1/g3
            _make_group_for_dedupe("g2", 8000.0, elevation_gain=40.0, name="Elm St"),
            # gain_per_km ~= 5 -> 'flat', same-ish distance as g1
            _make_group_for_dedupe("g3", 12500.0, elevation_gain=62.0, name="Elm St"),
            # gain_per_km > 30 -> 'hilly', clearly different terrain
            _make_group_for_dedupe("g4", 12000.0, elevation_gain=500.0, name="Elm St"),
        ]
        RouteAnalyzer._dedupe_route_names(groups)

        names = [g.name for g in groups]
        assert len(names) == len(set(names))
        # The hilly outlier is disambiguated by terrain alone.
        assert names[3] == "Elm St — hilly"
        # The three flat routes must be further split by distance bracket
        # (and ordinal, if two land in the same bracket).
        assert all(n.startswith("Elm St — flat") for n in names[:3])

    def test_final_verification_pass_catches_cross_group_collision(self):
        """
        A group that already carries a literal name (e.g. from a previous run)
        can coincidentally collide with a name generated by disambiguating a
        *different* original name group. The final verification pass must
        catch this and enforce uniqueness across the whole direction.
        """
        preexisting = _make_group_for_dedupe("g0", 15000.0, name="Home — long")
        # Two routes both originally named "Home"; their distances straddle
        # the median such that the 'long' bucket collides with `preexisting`.
        clashing = [
            _make_group_for_dedupe("g1", 9000.0, name="Home"),
            _make_group_for_dedupe("g2", 15000.0, name="Home"),
        ]
        groups = [preexisting] + clashing
        RouteAnalyzer._dedupe_route_names(groups)

        names = [g.name for g in groups]
        assert len(names) == len(set(names)), f"expected unique names, got {names}"

    def test_all_identical_routes_get_ordinal_suffixes(self):
        """When terrain and distance are identical, ordinal is the only lever."""
        groups = [_make_group_for_dedupe(f"g{i}", 12000.0, name="Home") for i in range(1, 4)]
        RouteAnalyzer._dedupe_route_names(groups)

        names = [g.name for g in groups]
        assert names[0] == "Home"
        assert names[1] == "Home #2"
        assert names[2] == "Home #3"
        assert len(names) == len(set(names))
