...
echo "\`\`\`bash"
echo "# macOS / Linux / Git Bash — output written directly to ISSUE_PRIORITIES.md"
echo "./scripts/update-issue-priorities.sh"
echo ""
echo "# Windows (PowerShell) — run via Git Bash, not WSL:"
echo "# WSL's git can't resolve a worktree's .git file when it holds a Windows-style"
echo "# absolute gitdir path (as git-for-windows writes), so it fails with"
echo "# 'fatal: not a git repository'. Git Bash's git is git-for-windows and handles"
echo "# this correctly."
echo "bash ./scripts/update-issue-priorities.sh"
echo "\`\`\`"
echo ""
