...
```
`gunicorn.conf.py` runs a single worker process with `worker_class = 'gthread'` (4 threads) rather than multiple sync workers — a slow/blocked request (e.g. a hanging Strava call) no longer serializes every other request behind it, without doubling the Pi's per-process memory footprint. The GHCR image is built **arm64-only** (`.github/workflows/docker-publish.yml`, #516) since only the Pi4 deploys the container; amd64 is commented out, not gone — re-enable it if an x86 deployment target shows up.
...
**Backend** (`launch.py` + `app/`): `launch.py` is a thin CLI entry point (start/stop/status/foreground) — all route handlers live in `app/api/*_bp.py` Blueprints, registered from `app/factory.py::create_app()` (the single app factory; the older `app/__init__.py`/`app/config.py` factory pair was deleted in #460). Business logic lives in `app/services/` — ten service classes: `AnalysisService`, `CommuteService`, `PlannerService`, `RouteLibraryService`, `WeatherService`, `TrainerRoadService`, `GarminService`, `ExplorationService` (ORS route/tile-coverage computation, cheapest-insertion route construction with overlap/out-and-back/wind-orientation and frontier/infill tile-collection strategies), `GeocodingService` (forward geocoding via Nominatim), `SettingsService` (user preference persistence). Services use a two-step init pattern: constructor then `.initialize()`. `app/container.py::ServiceContainer` inits them in parallel waves and exposes `refresh_services(*names)` (#461) to re-init just the service(s) affected by a settings change instead of resetting the whole container. The `maps_api` Blueprint (`app/api/maps_api.py`) serves map-tile endpoints under `/api/maps`; `app/api/activity_cache.py` is a small mtime-memoized cache of parsed activities shared by `stats_bp`/`data_bp`.

**Core analysis** (`src/`): The heavy lifting for route analysis. `src/route_analyzer.py` (~73KB) is the central engine using Fréchet distance (via `similaritymeasures`) for route similarity matching; it and `src/long_ride_analyzer.py` both delegate the actual comparison math to `src/route_comparison.py` (#507) — change similarity logic there, not in either caller. `src/data_fetcher.py` handles Strava API calls with rate-limit retry, including a full-history backfill mode (`backfill_full_history()`, `POST /api/fetch/backfill-history`, #486) that pages backward past the normal activity cap. `src/json_storage.py` is the current persistence layer (SQLite migration planned — Issue #131), with cross-process write locking on both POSIX (`fcntl.flock`) and Windows (`msvcrt.locking`, plus a rename retry for transient antivirus/indexer `PermissionError`s, #504). Activity data is cached under `data/cache/`. `legacy/report_generator.py` (moved out of `src/`, #467/#508) generates PDF/QR reports off the deprecated `main.py` CLI path only — its `weasyprint`/`qrcode` dependencies were dropped from `requirements.txt` and are manual-install extras.
...
- When editing UI: touch `templates/*.html` (or `static/commute.html`/`static/setup.html` for the two standalone pages), `static/js/*.js`, `static/css/main.css`, and the matching `app/api/*_bp.py` Blueprint for any API change.
